...
def _extract_parts(parts: list) -> list[dict]:
    ...
    for p in parts:
        ...
        if isinstance(root, TextPart) and getattr(root, "text", None):
            txt = root.text
            if "Memory service is not available" in txt or "Cannot add session to memory" in txt:
                continue
            ...
            if not parts and getattr(last_task, "history", None):
                for m in last_task.history:
                    if getattr(m, "role", None) == Role.agent:
                        # Skip memory warning messages if real response exists
                        extracted = _extract_parts(getattr(m, "parts", []))
                        for p in extracted:
                            if "Cannot add session to memory" not in p.get("text", ""):
                                parts.append(p)
